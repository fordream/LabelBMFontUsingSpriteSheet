...
bool HelloWorld::init()
{
    ...
    auto item2 = MenuItemImage::create(
                                       "CloseNormal.png",
                                       "CloseSelected.png",
                                       CC_CALLBACK_1(HelloWorld::createLabel, this));
...
//    this->addChild(label, 1);
	...
	frameCache->addSpriteFramesWithFile("sprites0.plist");
	/*
	auto testlabel = Label::createWithBMFont("messageLabel.fnt", "test string [abcdefghijk]");
	if(testlabel)
	{
		testlabel->setPosition(Vec2(origin.x + visibleSize.width/2,
									origin.y + visibleSize.height - testlabel->getContentSize().height));

//		testlabel->setWidth(100); // 横幅を指定して自動改行
...
void HelloWorld::createLabel(Ref* sender)
{
    ...
    Label* label = Label::createWithBMFont("messageLabel.fnt", "test test test test test test test");
    label->setMaxLineWidth(100);
    h += label->getContentSize().height;
    label->setPosition(origin + Vec2(visibleSize.width / 2, visibleSize.height - h));
    addChild(label);
}